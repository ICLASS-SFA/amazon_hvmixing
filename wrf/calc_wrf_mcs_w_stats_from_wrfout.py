...
def calc_w_prof(
    ...
    # # Temperature [K]
    # TK = TH * (1e5 / PRESSURE) ** (- R_dry / Cp_dry)
    # # Virtual temperature [K]
    # TV = TK * (1 + QV / 0.622) / (1 + QV)
    # # Moist air density [kg m-3]
    # Rho = PRESSURE / (R_dry * TV)

    # # Calculate mass flux (kg m-2 s-1)
    # MassFlux = (Rho * WA_ds).squeeze().load()
            ...
            if inpix_cloud > 0:
                ...
                with warnings.catch_warnings():
                    ...
                    if iZ3D_ds.ndim == 3:
                        ...
                        # Temperature [K]
                        iTK = iTH * (1e5 / iPRESSURE) ** (- R_dry / Cp_dry)
                        # Virtual temperature [K]
...
def main():
    ...
    # dsstats = dsstats.isel(tracks=slice(30,35))
